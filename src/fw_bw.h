        ...
        )
{
    ...
    while(true)
    {
        ...
        if(is_top_down)
        {
            ...
            {
                index_t *q = new index_t[queue_size];//新建q
                index_t head = 0;
                index_t tail = 0;
                //std::queue<index_t> q;
...
//        printf("tid, %d, %d, %d, %d\n", tid, step, queue_beg, queue_end);
        #pragma omp barrier
        if(is_top_down)//top down把temp的所有邻居放到thread
        {
        ...
        )
{
    ...
    index_t thread_queue_frontier=0;
...
//                }
                ...
                bool flag=false;
                fw_sa[v] = v;
                index_t v_beg = fw_beg_pos[v];
                index_t v_end = fw_beg_pos[v+1];

                for(; v_beg < v_end; ++v_beg)
                {
                    index_t w = fw_csr[v_beg];
                    if(w==v)
                    {
                        flag=true;
                        continue;
    ...
    //                    front_count ++;
                    ...
                    for(; my_beg < my_end; ++my_beg)
                    {
                        ...
                        if(scc_id[w] == 0 && fw_sa[w] == v)
                        {
                            ...
                            scc_id[w] = v;
                            mark[w]=4;
                            thread_queue[thread_queue_frontier++]=w;
                            thread_bin_size++;
                        }
                    }
                }
                if(flag){
                    thread_queue[thread_queue_frontier++]=v;
                    thread_bin_size++;
                    mark[v]=4;
                }
            }
