        ...
        )
{
    ...
    for(vertex_t fq_vert_id = vert_beg; fq_vert_id < vert_end; ++fq_vert_id)
    {
        ...
        {//
            ...
            if(MYTEST && vert_id==5)
            {
                std::cout<<"out_degree:"<<out_degree<<" out_neighbour:"<<out_neighbor<<std::endl;
            }
            ...
            if(out_degree == 1)
            {
                ...
                if(MYTEST && vert_id==5)
                {
                    std::cout<<"neighbor_out_degree:"<<neighbor_out_degree<<" neighbor_out_neighbor:"<<neighbor_out_neighbor<<std::endl;
                }
                ...
                if(neighbor_out_degree == 1 && neighbor_out_neighbor == vert_id)
                {
                    ...
                    thread_queue[thread_queue_frontier++]=out_neighbor;
                    fw_sa[vert_id]=0;
                    fw_sa[out_neighbor]=0;
                    if(MYTEST)
                    {
                        std::cout<<"trim 2:"<<vert_id<<" + "<<out_neighbor<<std::endl;
                    }
                    continue;
                }
            }
            ...
            if(in_degree == 1)
            {
               ...
               if(MYTEST && vert_id==5)
                {
                    std::cout<<"neighbor_in_degree:"<<neighbor_in_degree<<" neighbor_in_neighbor:"<<neighbor_in_neighbor<<std::endl;
                }
                ...
                if(neighbor_in_degree == 1 && neighbor_in_neighbor == vert_id)
                {
                    ...
                    if(MYTEST)
                    {
                        std::cout<<"trim 2:"<<vert_id<<" + "<<in_neighbor<<std::endl;
                    }
                    continue;
                }
            }
...
{
    vertex_t thread_queue_frontier=thread_bin[tid];
    index_t thread_bin_size=0;
    for(vertex_t fq_vert_id = vert_beg; fq_vert_id < vert_end; ++fq_vert_id)
    {
        vertex_t vert_id = small_queue[fq_vert_id];
            ...
            if(out_degree == 1)
            {
                ...
                if(B_out_degree == 1)
                {
                    ...
                    if(C_out_degree == 1 && C_out_neighbor == vert_id)
                    {
                        ...
                        mark[vert_id] = 3;
                        mark[out_neighbor] = 3;
                        mark[B_out_neighbor] = 3;
                        thread_bin_size+=3;
                        thread_queue[thread_queue_frontier++]=vert_id;
                        thread_queue[thread_queue_frontier++]=out_neighbor;
                        thread_queue[thread_queue_frontier++]=B_out_neighbor;
                        fw_sa[vert_id] = 0;
                        fw_sa[out_neighbor] = 0;
                        fw_sa[B_out_neighbor] = 0;
            ...
            if(in_degree == 1 && in_neighbor != vert_id)
            {
                ...
                if(B_in_degree == 1)
                {
                    ...
                    if(C_in_degree == 1 && C_in_neighbor == vert_id)
                    {
                        ...
                        mark[vert_id] = 3;
                        mark[in_neighbor] = 3;
                        mark[B_in_neighbor] = 3;
                        thread_bin_size+=3;
                        thread_queue[thread_queue_frontier++]=vert_id;
                        thread_queue[thread_queue_frontier++]=in_neighbor;
                        thread_queue[thread_queue_frontier++]=B_in_neighbor;  
                        fw_sa[vert_id] = 0;
                        fw_sa[in_neighbor] = 0;
                        fw_sa[B_in_neighbor] = 0;     
//                        size_3_type_1++;
                    }
                }
            }
            ...
            if(out_degree == 2)
            {
                ...
                if(B_out_degree == 1 && B_out_neighbor == vert_id)
                {
                    ...
                    if(C_out_degree == 1 && C_out_neighbor == vert_id)
                    {
                        ...
                        scc_id[out_neighbor_C] = -4;
                        mark[vert_id] = 3;
                        mark[out_neighbor_B] = 3;
                        mark[out_neighbor_C] = 3;
                        thread_bin_size+=3;
                        thread_queue[thread_queue_frontier++]=vert_id;
                        thread_queue[thread_queue_frontier++]=out_neighbor_B;
                        thread_queue[thread_queue_frontier++]=out_neighbor_C;
                        fw_sa[vert_id] = 0;
                        fw_sa[out_neighbor_B] = 0;
                        fw_sa[out_neighbor_C] = 0;
//                        size_3_type_2 ++;
                    }
                }
            }
            ...
            if(in_degree == 2)
            {
                ...
                if(B_in_degree == 1 && B_in_neighbor == vert_id)
                {
                    ...
                    if(C_in_degree == 1 && C_in_neighbor == vert_id)
                    {
                        ...
                        scc_id[in_neighbor_C] = -4;
                        mark[vert_id] = 3;
                        mark[in_neighbor_B] = 3;
                        mark[in_neighbor_C] = 3;
                        thread_bin_size+=3;
                        thread_queue[thread_queue_frontier++]=vert_id;
                        thread_queue[thread_queue_frontier++]=in_neighbor_B;
                        thread_queue[thread_queue_frontier++]=in_neighbor_C;
                        fw_sa[vert_id] = 0;
                        fw_sa[in_neighbor_B] = 0;
                        fw_sa[in_neighbor_C] = 0;
//                        size_3_type_2 ++;
                    }
                }
            }
